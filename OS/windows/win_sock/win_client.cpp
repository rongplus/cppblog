...

{
  ...
  // AF_INET = 2, The Internet Protocol version 4 (IPv4) address family, TCP
  // protocol
  ...
  ServerAddr.sin_addr.s_addr = inet_addr("4.227.145.213");
  ...
  if (shutdown(SendingSocket, SD_SEND) != 0)
    ...
    printf("Client: Well, there is something wrong with the shutdown().  The "
           "error code: %ld\n",
           WSAGetLastError());
